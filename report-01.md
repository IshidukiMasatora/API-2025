...
9. **並び替え（GET with Sort）** **追加課題**
    * クエリパラメータ sort_by=score&sort_order=desc を使って、score の降順で並び替えたデータを取得

10. **特定カラムのみ取得（GET with Select）** **追加課題**

    * クエリパラメータ select=name,email を使って、name と email のみを取得する。

11. **RESTの4原則対応表**
    * 上記10の設問について、下記の4原則のどれを満たしているかを表にまとめよ：

**対応表**
|番号|ステートレス|統一インターフェース|リソース指向|メソッドの意味|
|-|-|-|-|-|
|1|○|○|○|○|
|2|○|○|○|○|
|3|○|○|○|○|
...
1.	ステートレス → 各リクエストは独立しており、サーバーはセッション状態を保持しない
2.	統一インターフェース → 一貫したHTTPメソッド（GET, POST, DELETE）とリソース指向のURI
3.	リソース指向（URI設計）→ URIは「操作」ではなく「リソース」を表す
4.	HTTPメソッドの意味を守る → GETは取得、POSTは作成、DELETEは削除
